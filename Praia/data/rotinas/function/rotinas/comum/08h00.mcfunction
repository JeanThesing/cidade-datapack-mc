# ivanir na praia
function rotinas:rotinas/tp_npc {nome:"Seu Ivanir", coord:"59803.35 63.00 7891.49"}

# gerusa trabalhar
function rotinas:rotinas/tp_npc {nome:"Gerusa", coord:"59686 73.5 7934"}

# ronaldo do caldo trabalhar
function rotinas:rotinas/tp_npc {nome:"Ronaldo do Caldo", coord:"59768 64 7901"}

# tarcila trampo
function rotinas:rotinas/tp_npc {nome:"Tarcila", coord:"59654.77 74.00 7937.46"}

# Alberto Barros
function rotinas:rotinas/tp_npc {nome:"Alberto Barros", coord:"59683.32 73.06 7910.19"}

# Villa-Lobos
function rotinas:rotinas/tp_npc {nome:"Barbeiro Villa-Lobos", coord:"59605.25 81.00 8126.14"}
